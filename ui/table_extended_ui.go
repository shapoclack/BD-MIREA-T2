...
// UIRollupQuery - ROLLUP Aggregation
func UIRollupQuery(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	...
	columnsEntry.SetPlaceHolder("Столбцы для GROUP BY")

	aggregateEntry := widget.NewEntry()
	aggregateEntry.SetPlaceHolder("Агрегатная функция (COUNT, SUM, AVG)")

	form := widget.NewForm(
		widget.NewFormItem("Таблица", tableEntry),
		widget.NewFormItem("Группировка", columnsEntry),
		widget.NewFormItem("Агрегат", aggregateEntry),
	)

	dialog.ShowCustomConfirm("ROLLUP Aggregation", "Выполнить", "Отмена", form, func(ok bool) {
		if ok {
			// Парсим список столбцов
			columns := []string{}
			for _, col := range strings.Split(strings.TrimSpace(columnsEntry.Text), ",") {
				if c := strings.TrimSpace(col); c != "" {
					columns = append(columns, c)
				}
			}

			results, err := operation.ExecuteRollupQuery(ctx, pool,
				strings.TrimSpace(tableEntry.Text),
				columns,
				strings.TrimSpace(aggregateEntry.Text),
				"*")
...
func UICubeQuery(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	...
	dialog.ShowCustomConfirm("CUBE Aggregation", "Выполнить", "Отмена", form, func(ok bool) {
		if ok {
			// Парсим список столбцов
			columns := []string{}
			for _, col := range strings.Split(strings.TrimSpace(columnsEntry.Text), ",") {
			...
			results, err := operation.ExecuteCubeQuery(ctx, pool,
				strings.TrimSpace(tableEntry.Text),
				columns,
				"COUNT(*)",
				"*")
			...
			resultWindow := fyne.CurrentApp().NewWindow("CUBE Results")
			resultWindow.SetContent(container.NewScroll(resultTable))
			resultWindow.Resize(fyne.NewSize(800, 600))
			resultWindow.CenterOnScreen()
			resultWindow.Show()
...
// UICTEBuilder - WITH (CTE)
func UICTEBuilder(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	cteNameEntry := widget.NewEntry()
	cteNameEntry.SetPlaceHolder("Имя CTE")

	cteQueryEntry := widget.NewMultiLineEntry()
	cteQueryEntry.SetPlaceHolder("SQL запрос для CTE")
	cteQueryEntry.SetMinRowsVisible(3)

	mainQueryEntry := widget.NewMultiLineEntry()
	mainQueryEntry.SetPlaceHolder("Основной запрос с USE of CTE")
	mainQueryEntry.SetMinRowsVisible(3)

	form := container.NewVBox(
		widget.NewForm(
			widget.NewFormItem("Имя CTE", cteNameEntry),
		),
		widget.NewLabel("CTE Query:"),
		cteQueryEntry,
		widget.NewLabel("Main Query:"),
		mainQueryEntry,
	)

	dialog.ShowCustomConfirm("WITH (CTE)", "Выполнить", "Отмена", form, func(ok bool) {
		if ok {
			cteDef := operation.CTEDefinition{
				Name:  strings.TrimSpace(cteNameEntry.Text),
				Query: strings.TrimSpace(cteQueryEntry.Text),
			}

			qb := operation.NewQueryBuilder("")
			results, err := operation.ExecuteCTEQuery(ctx, pool,
				[]operation.CTEDefinition{cteDef},
				qb)
			...
			resultWindow := fyne.CurrentApp().NewWindow("CTE Results")
			resultWindow.SetContent(container.NewScroll(resultTable))
...
// UICreateView - Create VIEW
func UICreateView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	viewNameEntry := widget.NewEntry()
	viewNameEntry.SetPlaceHolder("Имя представления")
	viewQueryEntry := widget.NewMultiLineEntry()
	viewQueryEntry.SetPlaceHolder("SELECT запрос для представления")
	viewQueryEntry.SetMinRowsVisible(5)

	form := container.NewVBox(
		widget.NewForm(
			widget.NewFormItem("Имя VIEW", viewNameEntry),
		),
		widget.NewLabel("SELECT Query:"),
		viewQueryEntry,
	)

	dialog.ShowCustomConfirm("Create VIEW", "Создать", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.CreateView(ctx, pool,
				strings.TrimSpace(viewNameEntry.Text),
				strings.TrimSpace(viewQueryEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			showInfo(window, "VIEW успешно создано!")
		}
	}, window)
}

// UICreateOrReplaceView - Create or Replace VIEW
func UICreateOrReplaceView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	...
	dialog.ShowCustomConfirm("Create or Replace VIEW", "Создать/Обновить", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.CreateOrReplaceView(ctx, pool,
				strings.TrimSpace(viewNameEntry.Text),
				strings.TrimSpace(viewQueryEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
			...
			showInfo(window, "VIEW успешно обновлено!")
		}
	}, window)
}

// UIListViews - List all VIEWs
func UIListViews(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	views, err := operation.ListViews(ctx, pool)
	if err != nil {
		showError(window, "Ошибка: "+err.Error())
		return
	}
	...
	tableData = append(tableData, []string{"Представление"})
	for _, v := range views {
		tableData = append(tableData, []string{v})
	}

	resultTable, err := CreateTable(tableData)
	if err != nil {
		showError(window, err.Error())
		return
	}

	resultWindow := fyne.CurrentApp().NewWindow("VIEWs List")
	resultWindow.SetContent(container.NewScroll(resultTable))
...
// UIGetViewDefinition - Get VIEW definition
func UIGetViewDefinition(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	viewNameEntry := widget.NewEntry()
	viewNameEntry.SetPlaceHolder("Имя представления")
	form := widget.NewForm(
		widget.NewFormItem("VIEW", viewNameEntry),
	)

	dialog.ShowCustomConfirm("VIEW Definition", "Показать", "Отмена", form, func(ok bool) {
		if ok {
			def, err := operation.GetViewDefinition(ctx, pool, strings.TrimSpace(viewNameEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			defText := widget.NewMultiLineEntry()
			defText.SetText(def)
			defText.Disable()
			defText.SetMinRowsVisible(10)

			resultWindow := fyne.CurrentApp().NewWindow("VIEW Definition")
			resultWindow.SetContent(container.NewScroll(defText))
			resultWindow.Resize(fyne.NewSize(800, 500))
			resultWindow.CenterOnScreen()
			resultWindow.Show()
		}
	}, window)
...
// UIDropView - Drop VIEW
func UIDropView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	viewNameEntry := widget.NewEntry()
	viewNameEntry.SetPlaceHolder("Имя представления для удаления")
	form := widget.NewForm(
		widget.NewFormItem("VIEW", viewNameEntry),
	)

	dialog.ShowCustomConfirm("Drop VIEW", "Удалить", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.DropView(ctx, pool, strings.TrimSpace(viewNameEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			showInfo(window, "VIEW успешно удалено!")
		}
	}, window)
}

// UICreateMaterializedView - Create MATERIALIZED VIEW
func UICreateMaterializedView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	mvNameEntry := widget.NewEntry()
	mvNameEntry.SetPlaceHolder("Имя материализованного представления")
	mvQueryEntry := widget.NewMultiLineEntry()
	mvQueryEntry.SetPlaceHolder("SELECT запрос для MV")
	mvQueryEntry.SetMinRowsVisible(5)

	form := container.NewVBox(
		widget.NewForm(
			widget.NewFormItem("Имя MV", mvNameEntry),
		),
		widget.NewLabel("SELECT Query:"),
		mvQueryEntry,
	)

	dialog.ShowCustomConfirm("Create MATERIALIZED VIEW", "Создать", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.CreateMaterializedView(ctx, pool,
				strings.TrimSpace(mvNameEntry.Text),
				strings.TrimSpace(mvQueryEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			showInfo(window, "MATERIALIZED VIEW успешно создано!")
		}
	}, window)
}

// UIRefreshMaterializedView - Refresh MATERIALIZED VIEW
func UIRefreshMaterializedView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	mvNameEntry := widget.NewEntry()
	mvNameEntry.SetPlaceHolder("Имя MV для обновления")
	form := widget.NewForm(
		widget.NewFormItem("MV", mvNameEntry),
	)

	dialog.ShowCustomConfirm("Refresh MATERIALIZED VIEW", "Обновить", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.RefreshMaterializedView(ctx, pool, strings.TrimSpace(mvNameEntry.Text), false)
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			showInfo(window, "MATERIALIZED VIEW успешно обновлено!")
		}
	}, window)
}

// UIListMaterializedViews - List all MATERIALIZED VIEWs
func UIListMaterializedViews(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	mvs, err := operation.ListMaterializedViews(ctx, pool)
	if err != nil {
		showError(window, "Ошибка: "+err.Error())
		return
	}
	...
	var tableData [][]string
	tableData = append(tableData, []string{"Материализованное представление"})
	for _, mv := range mvs {
	...
	resultWindow := fyne.CurrentApp().NewWindow("MATERIALIZED VIEWs List")
	resultWindow.SetContent(container.NewScroll(resultTable))
	resultWindow.Resize(fyne.NewSize(700, 450))
	resultWindow.CenterOnScreen()
	resultWindow.Show()
}

// UIDropMaterializedView - Drop MATERIALIZED VIEW
func UIDropMaterializedView(ctx context.Context, pool *pgxpool.Pool, window fyne.Window) {
	mvNameEntry := widget.NewEntry()
	mvNameEntry.SetPlaceHolder("Имя MV для удаления")
	form := widget.NewForm(
		widget.NewFormItem("MV", mvNameEntry),
	)

	dialog.ShowCustomConfirm("Drop MATERIALIZED VIEW", "Удалить", "Отмена", form, func(ok bool) {
		if ok {
			err := operation.DropMaterializedView(ctx, pool, strings.TrimSpace(mvNameEntry.Text))
			if err != nil {
				showError(window, "Ошибка: "+err.Error())
				return
			}

			showInfo(window, "MATERIALIZED VIEW успешно удалено!")
		}
	}, window)
}
