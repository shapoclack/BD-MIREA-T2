...
type QueryBuilder struct {
	...
	joins           []JoinClause
	subqueries      []string
	windowFunctions []WindowFunction // слайс оконных функций для RANK/LAG/LEAD
}
...
func (qb *QueryBuilder) Build() string {
	...
	if len(qb.windowFunctions) > 0 {
		if len(qb.columns) > 0 || len(qb.aggregates) > 0 {
			query.WriteString(", ")
		}
		var windowFuncs []string
		for _, wf := range qb.windowFunctions {
			windowFuncs = append(windowFuncs, wf.buildSQL())
		}
		query.WriteString(strings.Join(windowFuncs, ", "))
	}
...
func (qb *QueryBuilder) Copy() *QueryBuilder {
	...
	newQB.windowFunctions = append(newQB.windowFunctions, qb.windowFunctions...)
	return newQB
}
