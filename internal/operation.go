...
func ListViews(ctx context.Context, pool *pgxpool.Pool) ([]string, error) {
	var views []string
	...
	rows, err := pool.Query(ctx, query)
	if err != nil {
		return nil, fmt.Errorf("ошибка при запросе к базе данных: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&viewName); err != nil {
			return nil, fmt.Errorf("ошибка при чтении представлений: %w", err)
		}
...
func ListMaterializedViews(ctx context.Context, pool *pgxpool.Pool) ([]string, error) {
	var views []string
	...
	for rows.Next() {
		var viewName string
		if err := rows.Scan(&viewName); err != nil {
			return nil, fmt.Errorf("ошибка при чтении материализованных представлений: %w", err)
		}
		views = append(views, viewName)
	}
